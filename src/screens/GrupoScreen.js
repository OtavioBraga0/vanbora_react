...
const GrupoScreen = ({navigation}) => {
    ...
    verifyDataList = (dataIn) => {
        let arrObjAlunos = dataList;   
        let exist = 0;   
        if(dataIn.length != 0){
            arrObjAlunos.forEach((element, index) => {      
                if(dataIn[0].key == element.key){       
                    ...
                    arrObjAlunos.splice(index, 1, dataIn[0])
                }
            })
        }

        if(exist == 0){
            arrObjAlunos.push(dataIn[0]);
            setDataList(arrObjAlunos)
        } else {
            setDataList(arrObjAlunos)
        }
    }
    ...
    init = () => {
        ...
        FirebaseService.getDataList(`grupo/${grupoId}/usuario` , dataIn => {
                ...
                <View style={styles.fullWidth}>
                    {
                        ...
                        dataList && dataList.map(
                            (item, index) => {
                                var presenca = "";
                                if(item.grupo[grupoId].presenca == "S"){
